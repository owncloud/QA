...
|8|Rename file|  1. Upload a image<br>2. Rename the image using special characters|1. Thumbnail is shown <br>2. Thumbnail is shown with the new name|||
|9|Duplicated name| 1. Upload an image<br>2. Rename another image with the same name in other folder <br>|1. Thumbnail is shown <br>2. Both thumbnails are correct|||
|10|Overwrite file in mobile device| 1. Upload an image<br>2. Upload another image with the same name in the folder<br>3. Solve the conflict with "overwrite"|1. Thumbnail is shown <br>2. A conflict is detected<br>3. The thumbnail of the new image is shown |||
|11|Overwrite file in server| 1. Switch the device connection off<br> 2. Upload an image to app<br>3. In server, upload another image with the same name<br>4. Switch the device connection on<br>5. Solve the conflict with "overwrite"|4. A conflict is detected<br>5. The thumbnail of device's version is shown |||
...
|13|Overwrite from a external app| 1. Upload an image<br>2. Upload another image from an external app with the same name<br>3. Solve the conflict with "overwrite"|1. Thumbnail is visible<br>2. A conflict is detected<br>3. The thumbnail of the second image is shown |||
|14|Rename file (conflict)| 1. Upload an image<br>2. Upload another image with the same name in the folder<br>3. Solve the conflict with "rename"|1. Thumbnail is shown <br>2. A conflict is detected<br>3. Both thumbnails are shown |||
|15|Delete file locally| 1. Upload an image<br>2. Download the image<br>3. Delete locally the image|1. Thumbnail is shown <br>2. Thumbnail is shown <br>3. Thumbnail is shown |||
