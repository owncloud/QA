...
| :----: | :-------- | :---- | :-------------- | :----: | :-------------- |
| **Correct view** ||||||
| 1 |  "Can share" not displayed | 1. Swipe to share a file or folder with federated users<br>2. Tap on (i) to manage privileges| "Can share" option is not displayed. Only "Can edit"| P m9 t8|t8: https: sharing a "x" failed, could not find "xx@docker…" maybe the server is currently unreachable|
...
| **Errors** ||||||
| 14 |  Inexistent User |  1. Repeat the test cases 2. or 3. using a username which does not exist in remote server  | The file can not be shared, an error appears |P m9||
| 15 |  Inexistent Server |  1. Repeat the test cases 2. or 3. using an inexistent remote URL | The file can not be shared, an error appears |P m9|Error message can be incorrect|
| 16 |  Share twice fed |  1. Share federated twice with the same user | First time the file is shared, second time, an error is received |P m9||
