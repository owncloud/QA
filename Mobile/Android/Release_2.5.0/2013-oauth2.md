...
|**Branding**|||||||
| 15 | OAuth2 URL hardcoded | Open the app.<br>Check both orientations | Correct view | P t5 m7 |  |
| 16 | OAuth2 URL hardcoded and hidden | Open the app.<br> Check both orientations | Correct view | P t5 m7 |  |
| 17 | Help link hidden | Open the app.<br>Check both orientations | Correct view | P t5 m7 |  |
|**OAuth2 UI flow**|||||||
...
| 48 | OAuth2 + basic | Attach an OAuth2 and a basic auth accounts to the same device | All correct | P t5 m7 |  | 
| 49 | OAuth2 + SAML | Attach an OAuth2 and a SAML auth accounts to the same device | Not posible | :lock: | Waiting for APK |
|**External actions**|||||||
| 50 | Refresh Token revoked | 1. After login, remove refresh token in DB<br> 2. Wait until session expires| Session is not refreshed. User redirected to login view | :lock: | Waiting fot APK |
