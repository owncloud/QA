...
#### PR: [https://github.com/owncloud/android/issues/3053]([https://github.com/owncloud/android/issues/3053])<br>


Device/s: Pixel2 v11 <br>
Server: 10.6
...
| **Basic Auth** |   |  |
| Log in Portrait | 1. Create a user<br>2. Log in owncloud enter a correct url<br>3. Type username + password  | Access to the app. Displayname (not the username) | AUTO |
...
| Wrong url | Log in owncloud by typing an incorrect url (such as  .serverurl.es) | It's not possible to access to owncloud. An error is shown | AUTO | 
| Empty credentials | 1. Type a correct url and let credentials empty | Log in button is hidden | P m11 m7
| Credentials error are detected | 1 Correct url<br>2. Log in owncloud with incorrect user or password |  Credential error is shown | AUTO |
| Username/Passwd with special character | 1. Create a user whose id is: e@some.es and the password: $h<br>2. Log in owncloud by typing a correct url<br>3. Type username + password,  | It's possible to access to owncloud | AUTO |
| Upercase url | Type a correct URL in uppercase.  | It's possible to access to owncloud | P m11 m7
| With blanks | Set some blanks after and before user name | Correct access | AUTO |
| LDAP with UID | Login with an LDAP user, that uses as id the UID | Correct access | P m11 m7
...
| URL with trailing slash | 1. Add a trailing slash to an correct URL | URL is correctly accepted | P m11 m7
...
|[3033](https://github.com/owncloud/android/issues/3033) ||| P m11 m7 
