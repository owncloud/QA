...
| :-------- | :---- | :------- | :----: | :------------------------- | 
|**Upload**||||||
| Upload one root | 1. Open FAB in root folder and select `Files`<br>2. Select one file from any provider | File is uploaded to root.<br>Uploads view shows the file uploaded to the correct place with correct size in the `Uploaded` section.<br>Notification appears while uploading and disappers after it finishes.|  F m12 | No notification. Intended? |
...
| Upload a very big file | 1. Open FAB and select `Files`<br>2. Select one file from any provider which size is bigger than 500MB | File is uploaded.<br>Uploads view shows the file uploaded to the correct place with correct size in the `Uploaded` section.<br>Notification appears while uploading and disappers after it finishes. | F m12 | fails with server timeout|
| Upload 1000 Files | 1. Open FAB and select `Files`<br>2. Select 1000 files from any provider. | Files are uploaded to the correct location.<br>Uploads view shows the list of 1000 files uploaded to the correct place with correct sizes in the `Uploaded` section.<br>Notification appears while uploading and disappers after every of them finishes. | F m12 | Chaos in uploads view|
| Only wifi | 1. In Settings, enable auto uploads for pictures with `only Wifi` option enabled<br>2. Remove wifi connection from device<br>3. Take some pictures and wait max 15 mins<br>4. Restablish connection| 3. Uploads enqueued<br>4.Uploads resumed and finished  | P m12 
| Charging | 1. In Settings, enable auto uploads for pictures with `Charging` option enabled<br>2. Remove wire from device<br>3. Take some pictures and wait max 15 mins<br>4. Restablish connection| 3. Uploads enqueued<br>4.Uploads resumed and finished  | P m12
| Only wifi & only charging | 1. In Settings, enable auto uploads for pictures with `only Wifi` and `Charging` options enabled<br>2. Remove wifi connection and wire from device<br>3. Take some pictures and wait max 15 mins<br>4. Restablish connection<br>5. Plug device to change| 3. Uploads enqueued<br>4. Uploads enqueued<br>5. Uploads resumed and finished  | P m12
| bigfilechunking false | 1. Select upload a file to a server with the capability `bigfilechunking false` (oCIS) | File uploaded with a single PUT | P m12
|**Upload view lists**||||||
...
| Clear Enqueued | 1. Upload a bunch of files<br>2. Remove the device connection before the files are being uploaded | Files are moved to Enqueued | P m12
...
|**Errors**||||||
| Lack of internet connection | 1. Remove connection from device<br>2. Upload a file| Upload fails<br>Failed upload in `Failed` section of uploads view with `Connection error`<br>Error in notification  | P m12
| Server down | 1. Switch server off<br>2. Upload a file| Upload fails<br>Failed upload in `Failed` section of uploads view with `Connection error` <br>Error in notification | P m12
| Maintenance mode | 1. Set server in maintenance mode<br>2. Upload a file| Upload fails<br>Failed upload in `Failed` section of uploads view with `Server unavailable`<br>Error in notification  | P m12 | Unknown error
| Deleted Folder | 1. Create a new folder and browse to it<br>2. Upload a file to that folder as target<br>3. Before submitting the upload: in web UI or other client, delete de target folder | Upload fails<br>Error is shown in a snackbar<br>Failed upload in `Failed` section of uploads view with `Folder error`<br>Error in notification | P m12 | Folder recreated
...
| Kill app | 1. Upload a big file<br>2. Before the upload finishes, kill the app | Upload fails<br>Failed upload in `Failed` section of uploads view with `App was terminated` |F m12 | Returns folder error
| Account disabled | 1. In the admin dashboard, go to Users and disable the account<br>2. Upload a file to the account | Upload fails<br>Failed upload in `Failed` section of uploads view with `Credentials error`<br>Error in notification  | P m12 |
