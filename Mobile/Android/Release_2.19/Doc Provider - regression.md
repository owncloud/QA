...
| Copy file to oC other oC account | From doc prov, copy a file to other oC account| Correct copied |  NA |
...
| Copy a file from other location (no oC) | From doc prov, copy files from other loation to oC| Nothing happened in oC | |  |
|**Folder**||||||
| Folder with subfolders oC | From doc prov, copy a folder with subfolders to the same account, different location| Correctly copied |  | With no subfolders works OK |
| Folder with subfolders oC other account | From doc prov, copy a folder with subfolders to other account | Correctly copied |  | With no subfolders works OK |
| Folder to other location | From doc prov, copy a folder with subfolders to other location| Correctly copied | F m9 | Not everything copied: https://github.com/owncloud/android/issues/2658 ¿? Something to check? |
...
|**Move File** |   |  |
|**File**||||||
| Move file to same oC account | From doc prov move a file to the same account, different location| Correctly copied |  |  |
| Move several files to same oC account | From doc prov move several files to the same account, different location| Correctly copied |  |  |
| Move file to other oC account | From doc prov move a file to the other oC account | Correctly copied |  |  |
| Move several files to other oC account | From doc prov move several files to other oC account | Correctly copied |  |  |
...
| Folder with subfolders oC other account | From doc prov move a folder with subfolders to other oC account | Correctly copied |  | With no subfolders works OK |
| Folder to other location | From doc prov, move a folder with subfolders to other location| Correctly moved | F m9 | Not everything copied: https://github.com/owncloud/android/issues/2658 ¿? Something to check? removed from oC |
| Copy several folders to other location | From doc prov, copy several folders to other location| Correctly moved (removed from oC) |  |  |
| Copy a folder from other location | From doc prov, copy some folders with subfolders from other location to oC | Correctly moved (created in oC) | |  |
|**Create File** |   |  |
