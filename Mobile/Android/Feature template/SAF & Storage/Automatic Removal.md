...
| Av. offline | 1. Set some files and folders as av. offline<br>2. In `Delete local copies`, select `1 hour`<br>4. Wait 2 hours<br>5. Unset the files and folders as av. offline withour deleting the local copy<br>6. Wait another hour | 4. Av. offline content is not deleted<br>5. Files and folder removed | |
...
| No accounts | 1. Remove all accounts in the device<br>2. In `Delete local copies`, select `1 hour`<br>3. Wait 2 hours | Nothing happens, since there are no files to remove | | 
...
| **Active files** |   |  |
| Upload manually | 1. Select `1 hour` in Setting<br>2. Just before 1h, upload manually a very big file (system will try to remove it while uploading)  | File is being synced in that moment, it shouldn't be removed locally |  | Difficult to reproduce
| Upload from 3rd party app | 1. Select `1 hour` in Setting<br>2. Just before 1h, share many files from a 3rd party app (system will try to remove it while uploading)  | Files being synced in that moment shouldn't be removed locally | | Difficult to reproduce
| Auto uploads | 1. Select `1 hour` in Setting<br>2. Enable auto uploads for videos<br>3. Just 15 mins before, record a video with the camera in order to be uploaded at the time the cleaning worker runs  | Files being synced in that moment shouldn't be removed locally | |  Difficult to reproduce
| Failed upload - Connection Enqueued | 1. Select `1 hour` in Setting<br>2. Remove device connection<br>3. Upload manually some files<br>4. Recover device connection<br>  | 3. Files are enqueued in uploads view<br>4. Files upladed correctly |   | 
