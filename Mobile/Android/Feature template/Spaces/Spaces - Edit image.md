...
| Edit image - big image |   1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on the space<br>3. Click on `Edit image`<br>4. Choose a image heavier than 7MB | Space is edited with the new image (check also in web).<br>Snackbar with `Space image updated correctly` |  | Server accepts but not updates. [Issue](https://github.com/owncloud/ocis/issues/11792) |  |
...
| No connection II | 1. Click on `Spaces` on the bottom bar<br>2. Click on 3-dot-button over a space with `manager` permissions<br>3. Click on `Edit image`<br>4. Remove connection from device<br>5. Choose image and submit, staying at spaces view<br>6. Recover connection | Image updated |  |  |  |
| No connection III | 1. Click on `Spaces` on the bottom bar<br>2. Click on 3-dot-button over a space with `manager` permissions<br>3. Click on `Edit image`<br>4. Remove connection from device<br>5. Choose image and submit<br>6. Browse to other view like uploads view<br>7. Recover connection | Image updated |  | Assumed known problem. Check: [[report]](https://github.com/owncloud/android/pull/4705#issuecomment-3490672287) [[explanation]](https://github.com/owncloud/android/pull/4705#issuecomment-3492313716) |  |
| No connection IV | 1. Click on `Spaces` on the bottom bar<br>2. Click on 3-dot-button over a space with `manager` permissions<br>3. Click on `Edit image`<br>4. Switch server off<br>5. Choose image and submit | Uploads view shows the file as failed<br>Error in snackbar `Image could`<br>` not be updated` |  |  |  |
