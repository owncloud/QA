...
| Cancel edition of expiration date  | 1. Click on Edit (pencil) on any member with expiration date<br>2. Open the date picker<br>3. `Cancel` | Member keeps the same expiration date. Check in web|   |  |
|**Errors**|  |
| No permissions | 1. Space manager opens `Members`<br>2. Click on Edit (pencil) over any member<br>3. Give new permission<br>4. Before submitting, manager is downgraded to editor/viewer<br>5. Click on `Edit` | Error `Member could not be edited`<br>` because permission error`| | |
