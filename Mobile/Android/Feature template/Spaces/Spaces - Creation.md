...
| No title | 1. Click on `Spaces` on the bottom bar<br>2. Click on FAB<br> | `Create`is disabled because no name was typed. `File name must`<br>` not be empty` | |  | 
| Long title | 1. Click on `Spaces` on the bottom bar<br>2. Click on FAB<br>3. Add a title longer than 255 chats | `Create` is disabled because title overflows. `Name must not be `<br>`longer than 255 characters` | |  | 
