...
| Edit space quota - enable |   1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on a space with `No Restriction`<br>3. Click on `Edit space`<br>4. Enable quota and add a quota value<br>5. Click on `Save`<br>6. Repeat with different quota values | Space is edited with the new quota value (check also in web).<br>Snackbar with `Space updated correctly` | |  |  |
| Edit space quota - disable |   1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on a space with a quoya value<br>3. Click on `Edit space`<br>4. Disable quota<br>5. Click on `Save` | Space is edited with the new quota value `No restriction` (check also in web).<br>Snackbar with `Space updated correctly` | |  |  |
...
| No quota | 1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on the space<br>3. Click on `Edit space`<br>4. Set quota with empty value | `Create`is disabled because no quota was typed. `Space quota must`<br>` not be empty` | |  | 
| Quota 0 | 1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on the space<br>3. Click on `Edit space`<br>4. Set quota with `0` value | `Create`is disabled because quota cannot be `0`. `Space quota must`<br>` not be empty` | |  | 
| Negative quota |1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on the space<br>3. Click on `Edit space`<br>4. Set quota with negative value | Not possible to enter negative values | |  | 
| Quota exceeded | 1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button on the space<br>3. Click on `Edit space`<br>4. Set quota with 1000001 GB | `Create`is disabled because quota exceeded maximum (1PB). `Space quota cannot exceed 1 PB` | | |  | 
...
| Spaces with different permission | 1. User has two spaces, one with edit permission and other with no edit pemission<br>2. Click on `Spaces` on the bottom bar<br>3. Click on the 3-dot-button on the first space<br>4. Click on the 3-dot-button on the second space |3. `Edit space` option is listed<br> 4. `Edit space` option is not listed |  |  |  |
| Quota not editable |  1. Click on `Spaces` on the bottom bar<br>2. Click on the 3-dot-button of a space in which the user is `manager`<br>3. Click on `Edit Space`| Quota section not available |  |  |  |
|**Role change**| |
| Add admin permission  | 1. Open `Spaces` of regular user with no `manager` permission over a space<br>2. In admin account, grant admin permissions to the user<br>3. Reopen the app and click on the 3-dot-button of any space<br>| 1. `Edit space` not available in the 3-dot-menu of the space <br>3. `Edit space` available in the 3-dot-menu of the space | |  |  |
