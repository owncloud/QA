...
| Exclude groups from sharing option active in oC Server and user B group is included; write down a pattern that matches with a regular excluded group | The group appears in the dropdown        | :construction:  17 | Allow username autocompletion in share dialog option disabled in oC Server; write down a pattern that matches with a regular group but not completed |
