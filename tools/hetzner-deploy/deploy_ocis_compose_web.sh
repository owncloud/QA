...
if [ "$1" = "-h" -o "$1" = "--help" ]; then
  ...
  echo "	env OCIS_DOCKER_IMAGE=owncloud/ocis OCIS_VERSION=master BASE_DOMAIN=ocis2-latest-DATE $0 ocm"
  echo "	env OCIS_VERSION=6.4.0 BASE_DOMAIN=fed1-640-DATE $0 ocm"
  echo ""
...
if [ -z "$OCIS_DOCKER_IMAGE" ]; then
  export OCIS_DOCKER_IMAGE=owncloud/ocis-rolling
fi
...
echo "stopping in the middle of the setup, study ~/web/docker-compose.yml and continue manually from here..."
exit 0
