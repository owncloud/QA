...
10 | Verify an empty folder with a filename longer than the allowed limit will not be synced | 1. Go to local sync folder 2. Create a single empty folder with a name longer than ??? -> TODO: find out the limit (client side: Windows 256 chars, server side: ?) 3. Look at the server repository 4. Repeat this with a folder at the root level, and in various subfolders up to 5 levels deep 5. Sync | The folder has not been synced to the server| :construction: | see https://github.com/owncloud/client/issues/9312#issuecomment-1000208288 |
