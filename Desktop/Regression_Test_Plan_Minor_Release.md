...
4 | Verify the permissions on new local folders | 1. Create a new local folder via "New Account Wizard" 2. Create a new folder on the new account via "Add Folder Sync Connection" | Verify that the new folders permissions are `drwx------` | :construction: | |
5 | Verify the permissions on existing local folders | 1. Use an existing local folder on the "New Account Wizard", pick any resolution 2. Use an existing folder with `777` permissions on the new "Folder Sync Connection" wizard | Verify that the existing folder permissions are `drwx------` | :construction: | |
6| Verify that you can sync five subfolders | 1. Go to local sync folder 2. Copy a folder with 5 subfolders 3. Wait for it to sync to Via Web |The folders are created on Via Website  |:construction: | |
7| Verify that you can sync five subfolders with files | 1. Go to local sync folder 2. Copy a folder with 5 subfolders + 10 files in each folder 3. Wait for it to sync to Via Web |The folder and the files are created on Via Website |:construction: | |
8| Verify that you can create multiples folders at once | 1. Go to local sync folder 2. Create 400 folders on the root 3. Wait for it to sync to Via Web |The folders are created on Via Website| :construction: | |
9| Verify that you can copy one folder | 1. Go to local sync folder 2. Create a single folder with some files in it 3. Copy and paste the folder 4. Wait for it to sync to Via Web |The folder is created on Via Website| :construction: | |
10| Verify that you can create a subfolder with long name | 1. Go to local sync folder (Desktop client) 2. Create a folder called "Folder1" 3. Create a subfolder called "LUsgzq!0k02sek+szBqrzN5=R#UJpWql&rwhnYVb~Gh!l!” 4. This subfolder had a file called ilppng.PNG inside it 5. let them sync |The files are synced correctly| :construction: | |
11| Verify pre existing folders in local (Desktop client) are copied over to Via Web on startup | 1. Turn off the Desktop client 2. Go to local sync folder 3. Create several folders inside the Desktop Client folder at several different levels 4. Turn ON the Desktop Client 5. Log into Via Web |At Via Web the folder has been synced over| :construction: | |
12| Verify that file(s) with invalid names are not synced | 1. Go to local sync folder 2. Create a single folder 3. Sync 4.Move a file or folder with an invalid name in the directory (e.g a/a) | The sync status folder represent the status as failed|:construction: | |
...
16| Invalid system names | 1. Navigate to the Via Web 2. Create a few folders with an invalid Windows sytem name: COM, AUX, COM1, LPT1, PRN, NULL |1. In Mac OS X the folders sync 2. In Windows OS the folders do not sync| :construction: | |
17| Invalid system names | 1. Navigate to the Via Web 2. Create a few folders with a special characters "%" in the name || :construction: | |
...
5| Long file name  | 1. Create a file with this name "dfkjsdfjksdkdfjsdfuidjfkdsjfksdjfksdjf ksdjfksdjfksdjfkdsjfksdjfkdsjfkdsfjlsdk fjsdkjflksdjfklsdjfksdjfkdsjfkldsjfkldsjf kdsjfksdjfksdjfklsdjfklsdjflksdjflksdjfk lsdjfklsdjfksdjfksdjfksdjfksdjfksd fjskdfjksdjfksdjfksdjfksdjfksd.pdf"|Sync is successful| :construction: | |
